...
for _prefix, _uri in WORD_OPENXML_NAMESPACES.items():
...
class OutputApplier:
    MINIMAL_PATCH_MAX_OPS = 80

    def _text_patch_ops(self, source_text: str, replacement_text: str) -> list[dict]:
        source = self._normalize_patch_text(source_text)
        replacement = self._normalize_patch_text(replacement_text)
            ...
            ops.append(
                {
                    ...
                    "start": source_start,
                    "end": source_end,
                    "text": replacement[replacement_start:replacement_end],
                }
            )
        if len(ops) > self.MINIMAL_PATCH_MAX_OPS:
            line_ops = self._line_patch_ops(source, replacement)
            if line_ops and len(line_ops) < len(ops):
                return line_ops
        return ops

    def _line_patch_ops(self, source: str, replacement: str) -> list[dict]:
        source_lines = source.splitlines(keepends=True)
        replacement_lines = replacement.splitlines(keepends=True)
        if not source_lines or not replacement_lines:
            return []
        ...
        for line in replacement_lines:
            replacement_offsets.append(replacement_offsets[-1] + len(line))

        matcher = difflib.SequenceMatcher(a=source_lines, b=replacement_lines, autojunk=False)
        ops = []
        for tag, source_start, source_end, replacement_start, replacement_end in matcher.get_opcodes():
            if tag == "equal":
                continue
            ops.append(
                {
                    "tag": tag,
                    "start": source_offsets[source_start],
                    "end": source_offsets[source_end],
                    "text": replacement[
                        replacement_offsets[replacement_start]:replacement_offsets[replacement_end]
                    ],
                }
            )
        return ops

    def _normalize_patch_text(self, text: str) -> str:
    ...
    def inspect_replace_availability(self, target: OutputTarget | None) -> tuple[bool, str | None]:
        ...
        if target.mode == "hwp":
            return (
                False,
                "HWP source replacement is temporarily disabled because the current COM patch path can damage document layout and formatting.",
            )
    ...
    def _apply_to_active_word(self, text: str, style_info: dict | None = None):
        ...
        self._log_word_replace(
            f"write modern-openxml text_len={len(str(text or ''))} newlines={str(text or '').count(chr(10))} "
            f"line_styles={len(line_styles)} segments={len(style_info.get('segments') or [])} "
            f"word_version={getattr(word, 'Version', '')!r} "
            f"sample={str(text or '')[:80]!r}"
        )
        source_text = str(style_info.get("_source_text") or "")
        if source_text:
            try:
                if self._apply_word_minimal_patch(document, source_text, text):
                    self._log_word_replace("minimal patch apply ok")
                    return
            except Exception as exc:
                self._log_word_replace(f"minimal patch apply failed: {type(exc).__name__}: {exc}")
                raise RuntimeError(
                    "Word minimal patch failed. Full-document replacement was blocked to protect images, tables, and formatting."
                ) from exc
            raise RuntimeError(
                "Word minimal patch was skipped because the change was too large. Full-document replacement was blocked to protect images, tables, and formatting."
            )
        try:
            self._apply_to_active_word_openxml(document, text)
            self._log_word_replace("modern-openxml apply ok")
            return
        except Exception as exc:
            self._log_word_replace(f"modern-openxml apply failed: {type(exc).__name__}: {exc}")
            self._apply_to_active_word_legacy(document, text, style_info)

    def _apply_to_active_word_legacy(self, document, text: str, style_info: dict):
        line_styles = style_info.get("line_styles") or []
        self._log_word_replace("legacy content-text apply start")
    ...
    def _apply_to_active_word_openxml(self, document, text: str):
        xml_text = getattr(document.Content, "WordOpenXML", "") or ""
        if not xml_text.strip():
            raise RuntimeError("Active Word document did not expose WordOpenXML.")
        replacement_xml = self._word_openxml_with_replacement(xml_text, text)
        if not replacement_xml:
            raise RuntimeError("WordOpenXML replacement produced no XML.")
        document.Content.InsertXML(replacement_xml)

    def _word_openxml_with_replacement(self, xml_text: str, text: str) -> str:
        root = ET.fromstring(xml_text)
        body = root.find(".//w:body", WORD_OPENXML_NAMESPACES)
        if body is None:
            raise RuntimeError("WordOpenXML body was not found.")

        paragraphs = body.findall(".//w:p", WORD_OPENXML_NAMESPACES)
        if not paragraphs:
            raise RuntimeError("WordOpenXML has no paragraphs to replace.")

        lines = self._word_replacement_lines(text)
        parent_map = {child: parent for parent in root.iter() for child in list(parent)}
        for index, paragraph in enumerate(paragraphs):
            line = lines[index] if index < len(lines) else ""
            self._replace_word_openxml_paragraph_text(paragraph, line)

        if len(lines) > len(paragraphs):
            self._append_word_openxml_extra_paragraphs(paragraphs[-1], parent_map, lines[len(paragraphs):])

        return ET.tostring(root, encoding="unicode")

    def _word_replacement_lines(self, text: str) -> list[str]:
    ...
    def _replace_word_openxml_paragraph_text(self, paragraph, line: str):
        text_nodes = paragraph.findall(".//w:t", WORD_OPENXML_NAMESPACES)
        if not text_nodes:
            if line:
                run = ET.SubElement(paragraph, f"{{{WORD_OPENXML_NAMESPACES['w']}}}r")
                text_node = ET.SubElement(run, f"{{{WORD_OPENXML_NAMESPACES['w']}}}t")
                self._set_word_openxml_text(text_node, line)
            return

        cursor = 0
        for index, text_node in enumerate(text_nodes):
            original_length = len(text_node.text or "")
            if index == len(text_nodes) - 1:
                chunk = line[cursor:]
            elif original_length > 0:
                chunk = line[cursor:cursor + original_length]
            else:
                chunk = ""
            self._set_word_openxml_text(text_node, chunk)
            cursor += len(chunk)

    def _append_word_openxml_extra_paragraphs(self, last_paragraph, parent_map: dict, extra_lines: list[str]):
        parent = parent_map.get(last_paragraph)
        if parent is None:
            return
        try:
            insert_at = list(parent).index(last_paragraph) + 1
        except ValueError:
            return
        template = copy.deepcopy(last_paragraph)
        for offset, line in enumerate(extra_lines):
            paragraph = copy.deepcopy(template)
            self._replace_word_openxml_paragraph_text(paragraph, line)
            parent.insert(insert_at + offset, paragraph)

    def _set_word_openxml_text(self, text_node, text: str):
        text_node.text = text
        xml_space = f"{{{WORD_OPENXML_NAMESPACES['xml']}}}space"
    ...
    def _apply_word_minimal_patch(self, document, source_text: str, replacement_text: str) -> bool:
        ops = self._text_patch_ops(source_text, replacement_text)
        if not ops:
            return True
        if self._patch_is_too_broad(ops, source_text):
            self._log_word_replace(
                f"minimal patch skipped: broad patch coverage={self._patch_coverage(ops, source_text):.2f}"
            )
            return False
        if len(ops) > self.MINIMAL_PATCH_MAX_OPS:
            self._log_word_replace(f"minimal patch skipped: too many ops count={len(ops)}")
            return False

        content = document.Content
        base_start = int(content.Start)
        for op in reversed(ops):
            start = base_start + op["start"]
            end = base_start + op["end"]
            word_range = document.Range(Start=start, End=end)
            word_range.Text = self._word_text_for_write(op["text"])
        return True
    ...
    def _apply_to_active_hwp(self, text: str, style_info: dict | None = None, window_handle: int | None = None):
        ...
        source_text = str(style_info.get("_source_text") or "")
        if source_text:
            current_text = self._read_hwp_text_snapshot(hwp, source_text)
            if not current_text:
                self._log_hwp_replace("minimal patch blocked: could not verify current HWP text")
                raise RuntimeError(
                    "Could not verify the current HWP document text. Source replacement was blocked to avoid editing the wrong range."
                )
            if not self._same_hwp_snapshot(current_text, source_text):
                self._log_hwp_replace(
                    "minimal patch blocked: HWP source mismatch "
                    f"expected_len={len(source_text)} current_len={len(current_text)} "
                    f"expected={source_text[:80]!r} current={current_text[:80]!r}"
                )
                raise RuntimeError(
                    "The HWP document text has changed since the correction was created. Run the correction again, then apply it."
                )
            try:
                if self._apply_hwp_minimal_patch(hwp, source_text, text):
                    self._log_hwp_replace("minimal patch apply ok")
                    return
            except Exception as exc:
                self._log_hwp_replace(f"minimal patch apply failed: {type(exc).__name__}: {exc}")
                raise RuntimeError(
                    "HWP minimal patch failed. Full-document replacement was blocked to protect images, tables, and formatting."
                ) from exc
            raise RuntimeError(
                "HWP minimal patch was skipped because the change was too large. Full-document replacement was blocked to protect images, tables, and formatting."
            )
    ...
    def _apply_hwp_minimal_patch(self, hwp, source_text: str, replacement_text: str) -> bool:
        ops = self._text_patch_ops(source_text, replacement_text)
        if not ops:
            return True
        if self._patch_is_too_broad(ops, source_text):
            ...
            return False
        if len(ops) > self.MINIMAL_PATCH_MAX_OPS:
            self._log_hwp_replace(f"minimal patch skipped: too many ops count={len(ops)}")
            return False

        if not self._hwp_patch_ops_are_same_length_replacements(ops):
            self._log_hwp_replace("minimal patch skipped: HWP only allows same-length replacements for now")
            raise RuntimeError(
                "HWP source replacement is currently limited to same-length character replacements. "
                "Length-changing edits were blocked to avoid damaging document layout or formatting."
            )

        for op in reversed(ops):
            start_pos = self._hwp_text_index_to_position(source_text, op["start"])
            end_pos = self._hwp_text_index_to_position(source_text, op["end"])
            deleted_length = op["end"] - op["start"]
            self._log_hwp_replace(
                "minimal patch same-length op "
                f"tag={op.get('tag')!r} range=({op['start']},{op['end']}) "
                f"pos={start_pos}->{end_pos} delete_len={deleted_length} insert_len={len(op['text'])} "
                f"insert={op['text'][:60]!r}"
            )
            self._apply_hwp_same_length_replacement(hwp, source_text, op)
        self._clear_hwp_selection(hwp)
        return True
    ...
    def _insert_hwp_text(self, hwp, text: str):
        if not text:
            return
        hwp.HAction.GetDefault("InsertText", hwp.HParameterSet.HInsertText.HSet)
        hwp.HParameterSet.HInsertText.Text = self._hwp_text_for_insert(text)
        hwp.HAction.Execute("InsertText", hwp.HParameterSet.HInsertText.HSet)

    def _clear_hwp_selection(self, hwp):
        try:
            hwp.Run("Cancel")
        except Exception:
            pass

    def _hwp_text_for_insert(self, text: str) -> str:
        return str(text or "").replace("\r\n", "\n").replace("\r", "\n")

    def _hwp_patch_ops_are_same_length_replacements(self, ops: list[dict]) -> bool:
        if not ops:
            return True
        for op in ops:
            if op.get("tag") != "replace":
                return False
            old_length = int(op.get("end", 0)) - int(op.get("start", 0))
            new_text = self._hwp_text_for_insert(str(op.get("text") or ""))
            if "\n" in new_text or old_length != len(new_text):
                return False
        return True

    def _apply_hwp_same_length_replacement(self, hwp, source_text: str, op: dict):
        start = int(op["start"])
        new_text = self._hwp_text_for_insert(str(op.get("text") or ""))
        for offset in range(len(new_text) - 1, -1, -1):
            source_index = start + offset
            start_pos = self._hwp_text_index_to_position(source_text, source_index)
            end_pos = self._hwp_text_index_to_position(source_text, source_index + 1)
            self._select_hwp_text_range(hwp, start_pos, end_pos, 1)
            self._insert_hwp_text(hwp, new_text[offset])

    def _read_hwp_text_snapshot(self, hwp, expected_text: str = "") -> str:
        first_text = ""
        for range_option in (0x0077, 0x0000, 0x000F, 0x0001):
            text = self._read_hwp_text_snapshot_once(hwp, range_option)
            if not text.strip():
                continue
            if not first_text:
                first_text = text
            if expected_text and self._same_hwp_snapshot(text, expected_text):
                return text
        return first_text

    def _read_hwp_text_snapshot_once(self, hwp, range_option: int) -> str:
        parts: list[str] = []
        try:
            hwp.InitScan(0x000F, range_option, 0, 0, -1, -1)
        except Exception as exc:
            self._log_hwp_replace(
                f"HWP verify InitScan failed range=0x{range_option:04X}: {type(exc).__name__}: {exc}"
            )
            return ""
        try:
            empty_streak = 0
            for _ in range(10000):
                try:
                    ...
                    self._log_hwp_replace(
                        f"HWP verify GetText failed range=0x{range_option:04X}: {type(exc).__name__}: {exc}"
                    )
                    break
                if chunk:
                ...
                if state in (0, 1):
                    break
                if state not in (2, 3) and not chunk:
                    break
                if empty_streak >= 8:
                    break
        finally:
            try:
                ...
                pass
        return self._normalize_patch_text("".join(parts))

    def _same_hwp_snapshot(self, left: str, right: str) -> bool:
        def normalize(value: str) -> str:
            return self._normalize_patch_text(value).rstrip("\n")

        return normalize(left) == normalize(right)

    def _patch_is_too_broad(self, ops: list[dict], source_text: str) -> bool:
        source = self._normalize_patch_text(source_text)
        if not source:
            return False
        if len(ops) == 1 and ops[0]["start"] <= 0 and ops[0]["end"] >= len(source):
            return True
        return len(source) >= 120 and self._patch_coverage(ops, source) > 0.65

    def _patch_coverage(self, ops: list[dict], source_text: str) -> float:
        source_len = max(1, len(self._normalize_patch_text(source_text)))
        changed = sum(max(0, int(op.get("end", 0)) - int(op.get("start", 0))) for op in ops)
        return changed / source_len
